...
def main():
	...
	tweets['bigdata'] = tweets['text'].apply(lambda tweet: word_in_text('bigdata', tweet))
	tweets['big data'] = tweets['text'].apply(lambda tweet: word_in_text('big data', tweet))
	tweets['tutorial'] = tweets['text'].apply(lambda tweet: word_in_text('tutorial', tweet))
	tweets['relevant'] = tweets['text'].apply(lambda tweet: word_in_text('bigdata', tweet) or word_in_text('tutorial', tweet) or word_in_text('big data', tweet))


	#Analyzing Tweets by programming language: Second attempt
	'''print 'Analyzing tweets by programming language: First attempt\n'
	tweets_by_prg_lang = [tweets[tweets['relevant'] == True]['python'].value_counts()[True], 
                      tweets[tweets['relevant'] == True]['bigdata'].value_counts()[True], 
                      tweets[tweets['relevant'] == True]['big data'].value_counts()[True]]
	x_pos = list(range(len(prg_langs)))
	width = 0.8
	fig, ax = plt.subplots()
	plt.bar(x_pos, tweets_by_prg_lang, width,alpha=1,color='g')
